...
The source-complete, deployment-gated `/api/intake/operator/*` routes expose held-for-review intake revisions to the local Founder Command adapter. Status, feed, and acknowledgement responses advertise `feed_contract: hyperion.intake.operator-feed/2.0`. Fixed `transport` metadata scopes `held_for_review` to the Worker delivery outbox, declares stable `(created_at, outbox_id)` ordering and bounded replay, and states that business-review state is not part of the contract. Each delivery acknowledgement names the exact outbox ID, revision hash, payload hash, local receipt ID, and outcome. Consumer receipts are append-only: exact replays are idempotent and changed replays conflict without an update. The route never changes the source `intake_outbox` state, represents Founder owner/SLA/review/approval state, or authorizes domain work.
