...
The source-complete, deployment-gated `/api/intake/operator/*` routes expose held-for-review intake revisions to the local Founder Command adapter. Status, feed, and acknowledgement responses advertise `feed_contract: hyperion.intake.operator-feed/2.2` and the additive `2.0`/`2.1` compatibility set. Consumers may request `2.0` or `2.1` with `X-Hyperion-Feed-Contract`; they must repeat that header when acknowledging the returned payload hash. Fixed `transport` metadata scopes `held_for_review` to the Worker delivery outbox, declares stable `(created_at, outbox_id)` ordering and bounded replay, and states that business-review state is not part of the contract. Feed 2.2 adds only bounded deterministic-receipt delivery metadata; recipient identity and rendered content are excluded. Each delivery acknowledgement names the exact outbox ID, revision hash, payload hash, local receipt ID, and outcome. Consumer receipts are append-only: exact replays are idempotent and changed replays conflict without an update. The route never changes the source `intake_outbox` state, represents Founder owner/SLA/review/approval state, or authorizes domain work.

After the immutable intake batch commits, the Worker attempts one deterministic receipt through Resend using a revision-derived idempotency key. Provider or delivery-ledger failure never rolls back the intake. Migration `0005_intake_acknowledgement_delivery.sql` stores only revision linkage, template/content hashes, provider reference, state, timestamps, and safe error codes. `/api/intake/webhooks/resend` verifies the raw-body Svix signature, rejects stale or invalid signatures, and deduplicates events by `svix-id` before recording delivered, bounced, or failed outcomes.
