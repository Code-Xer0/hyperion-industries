...
describe("Forge inbound mail routing", () => {
  it("delivers to Keshawn first and then copies Victor", async () => {
    ...
    await routeForgeMessage(
      ...
      { primary: "keshawn@example.net", operatorCopy: "victor@example.com" },
    );
  ...
  it("does not duplicate primary delivery when the operator copy fails", async () => {
    ...
    const forward = vi.fn(async (destination: string) => {
      if (destination === "victor@example.com") throw new Error("copy failed");
    });
